...
router.post("/register", upload.single("resume"), async(req, res) => {
    try {
        ...
        if (role === "trainer") {
            ...
            if (!demoVideoUrl || !expertise || experience === undefined || !bio) {
                return res.status(400).json({
                    message: "Trainers must provide demo video URL, expertise areas, experience years, and bio",
                });
            }
        ...
        if (role === "trainer") {
            return res.status(201).json({
                message: "Your trainer application has been submitted successfully! You will receive an email once your application is reviewed. Please check your email for updates.",
                status: "pending",
                email: user.email,
            });
        }
    ...
    try {
        ...
        if (user.role === "trainer" && !user.isApproved) {
            return res.status(403).json({
                message: "Your trainer account is still pending approval. Please wait for admin approval.",
                status: user.status,
            });
        }
